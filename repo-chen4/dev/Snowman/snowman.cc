...
//    body
    Matrix4 S_body_up = Matrix4::Scale(Vector3(1, 1, 1));
    Matrix4 T_body_up = Matrix4::Translation(Vector3(0, 2.3, 0));
    Matrix4 body_up_combo = S_body_up * T_body_up;
    quick_shapes_.DrawSphere(body_up_combo, view_matrix_, proj_matrix_, Color(1,1,1));
    
    Matrix4 S_body_down = Matrix4::Scale(Vector3(1.5, 1.5, 1.5));
    Matrix4 T_body_down = Matrix4::Translation(Vector3(0, 0, 0));
    Matrix4 body_down_combo = S_body_down * T_body_down;
    quick_shapes_.DrawSphere(body_down_combo, view_matrix_, proj_matrix_, Color(1,1,1));
    
//    left hand
    quick_shapes_.DrawLineSegment(identity, view_matrix_, proj_matrix_, Color(1,1,1), Point3(0,0,0), Point3(4, 0, 4), 1 );
