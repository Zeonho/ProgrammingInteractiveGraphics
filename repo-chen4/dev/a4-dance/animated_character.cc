                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    */
    Matrix4 steps = skeleton_.RotAxesSpaceToBoneSpace(bone_name) *
                    pose_.JointRotation(bone_name) *
					skeleton_.BoneSpaceToRotAxesSpace(bone_name);
    Matrix4 ctm = parent_transform * steps;
...
//    quick_shapes_.DrawAxes(ctm * S, view_matrix, proj_matrix);
    ...
    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
        R = Matrix4::RotationZ(M_PI/2);
        S = Matrix4::Scale(Vector3(0.02,0.13,0.02));
        quick_shapes_.DrawCylinder(ctm * R * S, view_matrix, proj_matrix, c);
        S = Matrix4::Scale(Vector3(0.03,0.03,0.03));
        if (bone_name == "lhipjoint" ) {
            T =Matrix4::Translation(Vector3(-4,0,0));
            quick_shapes_.DrawSphere(ctm * S * T, view_matrix, proj_matrix, c);
        } else {
            T =Matrix4::Translation(Vector3(4,0,0));
            quick_shapes_.DrawSphere(ctm * S * T, view_matrix, proj_matrix, c);
        }
    ...
    if (bone_name == "lfemur" || bone_name == "rfemur") {
        S = Matrix4::Scale(Vector3(0.02,0.15,0.02));
        T = Matrix4::Translation(Vector3(0, -0.5, 0));
        quick_shapes_.DrawCylinder(ctm * S * T, view_matrix, proj_matrix, c);
    }
    ...
    if (bone_name == "ltoes" || bone_name == "rtoes") {
        R = Matrix4::RotationX(M_PI/2);
        S = Matrix4::Scale(Vector3(0.01,0.03,0.01));
        quick_shapes_.DrawCylinder(ctm * R *S, view_matrix, proj_matrix, c);
    }
    if (bone_name == "lowerback") {
        S = Matrix4::Scale(Vector3(0.05,0.05,0.05));
        quick_shapes_.DrawCylinder(ctm * S, view_matrix, proj_matrix, c);
    }
    if (bone_name == "upperback") {
        S = Matrix4::Scale(Vector3(0.05,0.07,0.05));
        quick_shapes_.DrawCylinder(ctm * S, view_matrix, proj_matrix, c);
    }
    if (bone_name == "thorax") {
        S = Matrix4::Scale(Vector3(0.05,0.08,0.05));
        quick_shapes_.DrawCylinder(ctm * S, view_matrix, proj_matrix, c);
    }
    if (bone_name == "lowerneck" || bone_name == "upperneck") {
        S = Matrix4::Scale(Vector3(0.02,0.05,0.02));
        quick_shapes_.DrawCylinder(ctm * S, view_matrix, proj_matrix, c);
    }
    if (bone_name == "head") {
    ...
    if (bone_name == "lclavicle" || bone_name == "rclavicle") {
        R = Matrix4::RotationZ(M_PI/2);
        S = Matrix4::Scale(Vector3(0.02,0.1,0.02));
        quick_shapes_.DrawCylinder(ctm * R * S, view_matrix, proj_matrix, c);
    }
...
//        quick_shapes_.DrawCylinder(ctm * R *  S, view_matrix, proj_matrix, c);
        S = Matrix4::Scale(Vector3(0.03,0.03,0.03));
        quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, c);
        if (bone_name == "lhumerus" || bone_name == "lradius") {
            R = Matrix4::RotationZ(M_PI/2);
            S = Matrix4::Scale(Vector3(0.02,0.13,0.02));
            T = Matrix4::Translation(Vector3(0, -1.2, 0));
            quick_shapes_.DrawCylinder(ctm * R * S * T, view_matrix, proj_matrix, c);
        } else if (bone_name == "rhumerus" || bone_name == "rradius") {
            R = Matrix4::RotationZ(M_PI/2);
            S = Matrix4::Scale(Vector3(0.02,0.13,0.02));
            T = Matrix4::Translation(Vector3(0, 1.2, 0));
            quick_shapes_.DrawCylinder(ctm * R * S * T, view_matrix, proj_matrix, c);
