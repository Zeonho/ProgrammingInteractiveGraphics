...
void QuakeApp::OnGlobeBtnPressed() {
    ...
    earth_.setGlobeMode(GlobeMode);
    if (GlobeMode) {
        earth_.DrawGlobe(20, 20);
    } else {
        earth_.DrawPlane(10, 10);
    }
    GlobeMode = !GlobeMode;
}
...
void QuakeApp::DrawUsingOpenGL() {
    ...
    for (int x=start; x<end; x++) {
        ...
        double longtitude = e.longitude();
        double magnitude = e.magnitude();
        earth_.DrawEarthquake(longtitude, latitude, magnitude, model_matrix, view_matrix_, proj_matrix_);
    }
