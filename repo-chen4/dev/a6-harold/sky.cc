                       ...
                       const Mesh &stroke2d_mesh, const Color &stroke_color)
{
		...
		// The new mesh will then have the exact same vertices and indices as the original.
		// We want to keep the same indices, but the vertices need to be altered.
		...
		for (int i = 0; i < m.num_vertices(); i++) {
			pt = Point2(m.vertex(i).x(), m.vertex(i).y());
			ScreenPtToSky(view_matrix, proj_matrix, pt, &skypoint);
			vertices.push_back(skypoint);
		}
