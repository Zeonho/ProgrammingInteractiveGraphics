                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    // ground plane.
	Vector3 projection_plane;
	Vector3 projection_plane_normal;
	Point3 S;
	Point3 E;
	
	ScreenPtToGround(view_matrix, proj_matrix, stroke2d[0], &S);
	ScreenPtToGround(view_matrix, proj_matrix, stroke2d[stroke2d.size() - 1], &E);
	projection_plane = E - S;
	projection_plane_normal = (projection_plane.Cross(Vector3(0,1,0))).ToUnit();
	...
	for (int i = 0; i < stroke2d.size(); i++) {
		Point3 pointOnPlane;
		// ScreenPtToGround(view_matrix, proj_matrix, stroke2d[i], &ground_point);
		Point3 pt3d = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, stroke2d[i]);
		Ray ray(eye, (pt3d - eye).ToUnit());
		float t;
		ray.IntersectPlane(S, projection_plane_normal, &t, &pointOnPlane);
		silhouette_curve.push_back(pointOnPlane);
	}
	...
	//		std::cout << S << "\n";
	//		std::cout << E << "\n";
	//		std::cout << projection_plane << "\n";
	//		std::cout << projection_plane_normal << "\n";
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
		...
		Point3 closest_point_in_plane = P.ClosestPointOnPlane(S, projection_plane_normal);
		float h = hfunc(projection_plane_normal, silhouette_curve, closest_point_in_plane);
		float Py = P.y();
		
		
		// Debug
		// std::cout << closest_point_in_plane << "\n";
		// std::cout << "Py: " << Py;
		// std::cout << " h: " << h << "\n";
		if (h != 0) {
			float d = P.DistanceToPlane(S, projection_plane_normal);
			
			// Calculate w(d)
			float n = 1 -  pow((d/5.0),2);
			float w;
