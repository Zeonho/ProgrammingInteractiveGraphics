...
float circleX(float x, float y) {
	float theta = atan2(y, x);
	float r = pow((pow(x, 2) + pow(y, 2)), 0.5);
	return r * cos(theta);
}
float circleY(float x, float y) {
	float theta = atan2(y, x);
	float r = pow((pow(x, 2) + pow(y, 2)), 0.5);
	return r * sin(theta);
}

Point2 circle(Point2 p) {
	float x = p.x();
	float y = p.y();
	return Point2(circleX(x,y), circleY(x, y));
}
...
void Example::InitOpenGL() {
    ...
    tex_coords.push_back(circle(Point2(0,1)));
    tex_coords.push_back(circle(Point2(1,1)));
    tex_coords.push_back(circle(Point2(1,0)));
    tex_coords.push_back(circle(Point2(0,0)));
	std::cout << circle(Point2(0,1)) << "\n";
